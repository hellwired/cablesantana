 ...
 * user_model.php
 *
 * Este archivo contiene las funciones para interactuar con la tabla 'usuarios'
...
function createUser($nombre_usuario, $contrasena, $rol, $email = null, $cliente_id = null) {
    ...
    $stmt = $conn->prepare("INSERT INTO usuarios (nombre_usuario, contrasena, email, rol, cliente_id) VALUES (?, ?, ?, ?, ?)");
    if (!$stmt) {
    ...
    $stmt->bind_param("ssssi", $nombre_usuario, $hashed_password, $email, $rol, $cliente_id);
    ...
    if ($stmt->execute()) {
        ...
        if ($conn->errno == 1062) { // Código de error para entrada duplicada
            error_log("Error de duplicidad al crear usuario: " . $conn->error);
        } else {
            error_log("Error al ejecutar la inserción de usuario: " . $conn->error);
        }
        $stmt->close();
        closeDB($conn);
        return false;
...
function getUserById($id) {
    ...
    $stmt = $conn->prepare("SELECT id, cliente_id, nombre_usuario, contrasena, email, rol, fecha_creacion, activo FROM usuarios WHERE id = ?");
    if (!$stmt) {
...
function getUserByUsername($nombre_usuario) {
    ...
    $stmt = $conn->prepare("SELECT id, cliente_id, nombre_usuario, contrasena, email, rol, fecha_creacion, activo FROM usuarios WHERE nombre_usuario = ?");
    if (!$stmt) {
...
function getAllUsers() {
    ...
    $sql = "SELECT id, cliente_id, nombre_usuario, email, rol, fecha_creacion, activo FROM usuarios";
...
function updateUser($id, $data) {
    ...
    $sql = "UPDATE usuarios SET " . implode(', ', $set_clauses) . " WHERE id = ?";
...
function verifyUser($nombre_usuario, $contrasena) {
    ...
    $stmt = $conn->prepare("SELECT id, cliente_id, nombre_usuario, contrasena, email, rol, activo FROM usuarios WHERE nombre_usuario = ? AND activo = TRUE");
    if (!$stmt) {
    ...
    if ($result->num_rows === 1) {
        ...
        if ($password_match) {
            ...
            if ($needs_rehash) {
                ...
                $update_stmt = $conn->prepare("UPDATE usuarios SET contrasena = ? WHERE id = ?");
                if ($update_stmt) {
                    $update_stmt->bind_param("si", $new_hashed_password, $user['id']);
                    $update_stmt->execute();
                    $update_stmt->close();
                    logAuditAction($user['id'], 'Contraseña actualizada automáticamente a nuevo hash', 'usuarios', $user['id']);
                } else {
            ...
            logAuditAction($user['id'], 'Inicio de sesión exitoso', 'usuarios', $user['id']);
            $stmt->close();
            closeDB($conn);
            return $user;
        }
...
function getUserByEmail($email) {
    ...
    $stmt = $conn->prepare("SELECT id, cliente_id, nombre_usuario, email, rol, activo FROM usuarios WHERE email = ?");
    if (!$stmt) {
        error_log("Error al preparar la consulta de obtención de usuario por email: " . $conn->error);
        closeDB($conn);
        return null;
    }
